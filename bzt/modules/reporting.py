...
class CustomStatus(Reporter, AggregatorListener, FunctionalAggregatorListener):
    """
    A reporter copy of Final status with the name changed.
    """

    def __init__(self):
        self.log.info("init:customReport")
    ...
    def startup(self):
        self.log.info("startup:customReport")
        self.start_time = time.time()

    def prepare(self):
        self.log.info("prepare:customReport")
        #super(CustomStatus, self).prepare()
        if isinstance(self.engine.aggregator, ResultsProvider):
            self.engine.aggregator.add_listener(self)
        elif isinstance(self.engine.aggregator, FunctionalAggregator):
            self.engine.aggregator.add_listener(self)
    ...
    def aggregated_second(self, data):
        self.log.info("aggregated_second:customReport")
        """
        Just store the latest info
        :type data: bzt.modules.aggregator.DataPoint
        """
